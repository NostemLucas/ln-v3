<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="flex h-screen overflow-hidden">
      <!-- Panel lateral izquierdo con tabs para componentes y plantillas -->
      <div
      ...
      >
        ...
        <!-- Tabs para componentes y plantillas -->
        ...
        <div class="p-4 border-t border-gray-200 bg-gray-50 space-y-2">
          ...
          >
            ...
            {{
              showPreviewPanel ? "Ocultar Vista Previa" : "Mostrar Vista Previa"
            }}
          </button>
        </div>
      </div>

      <!-- Área principal de contenido con editor y vista previa -->
      <div class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <!-- Área de trabajo con scroll, panel de propiedades y vista previa -->
        <div class="flex-1 flex overflow-hidden">
          <!-- Área de trabajo principal y vista previa -->
          <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
            <!-- Editor principal -->
            <div
              class="flex-1 overflow-y-auto p-6 bg-gray-50"
              :class="{ 'md:w-1/2': showPreviewPanel }"
            >
            ...
            <div
              ...
              class="md:w-1/2 border-l border-gray-200 bg-white overflow-y-auto"
            >
              <div
              ...
              >
                <h2 class="font-medium text-gray-800 flex items-center gap-2">
                  <Icon name="lucide:eye" class="h-4 w-4 text-emerald-600" />
                ...
                <div v-if="selectedBlock">
                  ...
                  >
                    <div class="mb-4">
                      ...
                      <div v-if="selectedBlock.type === 'text'" class="mb-2">
                        <div
                          class="flex flex-wrap gap-2 mb-2 p-2 border rounded-md bg-gray-50"
                        >
                          <button
                            @click="insertTextFormat('bold')"
                          ...
                          <button
                            @click="insertTextFormat('underline')"
                            class="p-1.5 border rounded-md hover:bg-gray-100"
                            title="Subrayado"
                          >
                        ...
                        >
                          <div class="grid grid-cols-8 gap-1">
                            <button
                              ...
                              :key="color.value"
                              @click="insertColoredText(color.value)"
                          ...
                          <div class="mt-2 flex items-center gap-2">
                            ...
                            />
                            <button
                              @click="insertColoredText(customColor)"
                  ...
                  <div v-if="selectedBlock.type === 'list'">
                    ...
                    <div class="mb-4">
                      ...
                      <div
                        v-for="(item, index) in selectedBlock.listProps
                          ?.items || []"
                        :key="index"
                  ...
                  <div v-if="selectedBlock.type === 'code'">
                    ...
                    <div class="mb-4">
                      ...
                      <select
                        :value="
                          selectedBlock.codeProps?.language || 'javascript'
                        "
                        class="w-full p-2 border rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
                        @change="
                          updateCodeProps({
...
const initGridStack = () => {
  ...
  gridStack.value.on("change", () => {
    ...
    if (showPreviewPanel.value && previewGridStack.value) {
      renderPreviewBlocks();
    }
  });
...
`;
  ...
  if (block.type === "title") {
    content += `<div class="block-content font-bold text-2xl md:text-3xl outline-none w-full min-h-[40px] ${textStyles}" contenteditable style="${
      block.textProps?.color ? `color: ${block.textProps.color};` : ""
    }">${processTextWithSpans(block.content) || "Ingrese título aquí"}</div>`;
    ...
    content += `<div class="block-content font-medium text-xl md:text-2xl outline-none w-full min-h-[36px] ${textStyles}" contenteditable style="${
      ...
      processTextWithSpans(block.content) || "Ingrese subtítulo aquí"
    }</div>`;
  } else if (block.type === "text") {
    content += `<div class="block-content text-base outline-none w-full min-h-[100px] ${textStyles}" contenteditable style="${
      block.textProps?.color ? `color: ${block.textProps.color};` : ""
    }">${processTextWithSpans(block.content) || "Ingrese texto aquí"}</div>`;
  ...
  `;
        ...
        <div class="column-content outline-none h-full" data-column-index="${i}" contenteditable>
          ${
            block.columnContent && block.columnContent[i]
              ? processTextWithSpans(block.columnContent[i])
    ...
    if (listType === "bullet") {
      ...
      (block.listProps?.items || ["Elemento 1", "Elemento 2"]).forEach(
        (item) => {
          content += `<li>${processTextWithSpans(item)}</li>`;
        }
      );
      content += "</ul>";
      ...
      (block.listProps?.items || ["Elemento 1", "Elemento 2"]).forEach(
        (item) => {
          ...
          <li class="flex items-start">
            ...
            <span>${processTextWithSpans(item)}</span>
          </li>
        `;
        }
      );
    ...
    if (quoteStyle === "default") {
      content += `
        <blockquote class="border-l-4 border-gray-300 pl-4 italic text-gray-700">
          <p>${
            processTextWithSpans(block.content) ||
            "Ingrese texto de la cita aquí"
          }</p>
          ${
            ...
            ${data
              .map(
                ...
                ${row
                  .map(
                    (cell: any) =>
                      `<td class="border border-gray-300 px-4 py-2">${processTextWithSpans(
                        cell
                      )}</td>`
                  )
                  .join("")}
...
// Procesar texto con spans formateados
const processTextWithSpans = (text: string | undefined): string => {
  if (!text) return "";

  // Buscar patrones como <b>texto</b>, <i>texto</i>, <u>texto</u>, <span style="color:#HEX">texto</span>
  ...
  // Mantener spans con estilos de color
  processedText = processedText.replace(
    /<span\s+style="color:(#[0-9a-fA-F]{3,6}|[a-zA-Z]+)">(.*?)<\/span>/g,
...
// Insertar formato de texto en la posición del cursor
const insertTextFormat = (format: "bold" | "italic" | "underline") => {
  if (!textEditor.value) return;

  const start = textEditor.value.selectionStart;
  const end = textEditor.value.selectionEnd;
  const selectedText = textEditor.value.value.substring(start, end);
  ...
  const newText =
    textEditor.value.value.substring(0, start) +
    formattedText +
    textEditor.value.value.substring(end);

  updateBlockContent(newText);

  // Restaurar el foco después de la actualización
  nextTick(() => {
    if (textEditor.value) {
      textEditor.value.focus();
      textEditor.value.setSelectionRange(
...
// Insertar texto coloreado en la posición del cursor
...
const renderPreviewBlock = (block: ContentBlock) => {
  ...
  if (block.type === "title") {
    content += `<div class="font-bold text-2xl md:text-3xl w-full ${textStyles}" style="${
      block.textProps?.color ? `color: ${block.textProps.color};` : ""
    }">${processTextWithSpans(block.content) || "Título"}</div>`;
  } else if (block.type === "subtitle") {
    content += `<div class="font-medium text-xl md:text-2xl w-full ${textStyles}" style="${
      block.textProps?.color ? `color: ${block.textProps.color};` : ""
    }">${processTextWithSpans(block.content) || "Subtítulo"}</div>`;
  } else if (block.type === "text") {
    content += `<div class="text-base w-full ${textStyles}" style="${
      block.textProps?.color ? `color: ${block.textProps.color};` : ""
    }">${processTextWithSpans(block.content) || "Texto"}</div>`;
  } else if (block.type === "divider") {
    const dividerProps = block.dividerProps || {
      style: "solid" as DividerStyle,
      thickness: 1,
...
const addComponent = (componentType: ComponentType) => {
  ...
  if (gridStack.value) {
    ...
    nextTick(() => {
      ...
      // Inicializar la vista previa si está activa
      if (showPreviewPanel.value) {
        initPreviewGridStack();
      }
    });
...
// Observar cambios en fixedFields para actualizar la vista previa
watch(
  fixedFields,
  () => {
    if (showPreviewPanel.value && previewGridStack.value) {

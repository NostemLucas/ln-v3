<template>
	<div class="min-h-screen bg-gray-50 p-4">
		...
		>
			<LayoutTemplate class="h-16 w-16 text-gray-300 mx-auto mb-4" />
...
<script setup lang="ts">
	...
	import { LayoutTemplate, Upload } from 'lucide-vue-next';
	...
	const renderBlock = (block: ContentBlock) => {
		...
		if (block.type === 'title') {
			content += `<div class="font-bold text-2xl md:text-3xl">${block.content || 'Título'}</div>`;
		} else if (block.type === 'subtitle') {
			content += `<div class="font-medium text-xl md:text-2xl">${block.content || 'Subtítulo'}</div>`;
    ...
    `;
			...
			content += `
      <div class="w-full grid h-full" style="grid-template-columns: repeat(${block.columns || 2}, 1fr); gap: 1rem;">
    `;
        ...
        <div class="p-2">
          <div>
            ${block.columnContent && block.columnContent[i] ? block.columnContent[i] : `Contenido columna ${i + 1}`}
          </div>
        </div>
      `;
			}
